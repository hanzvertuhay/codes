...
from bip39_data import WORDSET as BIPSET, check as bip39_check
...
try:
    ...
    QT_AVAILABLE = False
    QT_IMPORT_ERROR = exc
    QApplication = QWidget = QVBoxLayout = QHBoxLayout = QLabel = QLineEdit = QPushButton = None  # type: ignore
    QCheckBox = QListWidget = QProgressBar = QFileDialog = QMessageBox = QListWidgetItem = None  # type: ignore
    QSplitter = QPlainTextEdit = QMenuBar = QStatusBar = QAbstractItemView = QMenu = None  # type: ignore
    QThread = Signal = QUrl = Qt = QDesktopServices = QIcon = QAction = None  # type: ignore
...
# BIP39 helpers (word set and checksum verifier) come from bundled wordlist.
...
def find_phrases_robust(text: str) -> Tuple[List[str], List[str], List[str]]:
    ...
    Возвращает три списка:
      valid_phrases    — с корректной BIP-39 checksum,
      near_phrases     — из слов BIP-39, но не проходящие checksum (для справки),
      electrum_phrases — 12/24 слова с корректной Electrum checksum.
    ...
    for phrase in candidates:
        ...
        if all((w in BIPSET) for w in words):
            if bip39_check(phrase):
                ...
                if len(words) in ELECTRUM_LENGTHS and is_electrum_new_seed(phrase):
                    electrum.append(phrase)
                else:
                    near.append(phrase)
            continue
        # Electrum path: allow 12/24 english words that pass Electrum checksum
        if len(words) in ELECTRUM_LENGTHS and all(re.fullmatch(r"[A-Za-z]+", w) for w in words):
            if is_electrum_new_seed(phrase):
                electrum.append(phrase)

    return valid, near, electrum
def is_suspicious(phrase: str) -> bool:
    ...
    if bigram_rep_ratio >= MAX_BIGRAM_REPEAT_RATIO:
        return True
    if max_freq > MAX_ALLOWED_DUPLICATES:
        return True
...
# -------------------- Worker --------------------
def scan_file(path_str: str) -> Tuple[str, List[str], List[str], List[str], List[tuple[str,str]], List[str]]:
    text = extract_text(Path(path_str))
    if not text:
        return path_str, [], [], [], [], []
    valid, near, electrum = find_phrases_robust(text)
    suspicious_valid = [v for v in valid if is_suspicious(v)]
    suspicious_electrum = [e for e in electrum if is_suspicious(e)]
    suspicious = suspicious_valid + suspicious_electrum
    valid_clean = [v for v in valid if v not in suspicious_valid]
    electrum_clean = [e for e in electrum if e not in suspicious_electrum]
    eth = find_eth_keys(text) if os.environ.get('FIND_ETH','0') == '1' else []
    return path_str, valid_clean, near, suspicious, eth, electrum_clean
...
if QT_AVAILABLE:
        ...
        def run(self):
            ...
            try:
                with Pool(procs) as pool:
                    ...
                    try:
                        for path, valid, near, suspicious, eth, electrum in iterator:
                            if self._stop:
                                pool.terminate()
                                try:
                                    pool.join()
    ...
    class Main(QWidget):
        ...
        def start_scan(self):
            ...
            self.list_valid.clear(); self.list_near.clear(); self.list_susp.clear(); self.log.clear(); self.log_electrum.clear()
            self.count_seed = {length: 0 for length in VALID_LENGTHS}
        ...
        def on_found_near(self, phrase: str, path: str):
            # Only add to the Near list if SHOW_NEAR is enabled. Electrum seeds are handled separately.
            ...
            try:
                if self.chk_typo12.isChecked() and len(phrase.split()) == 12:
                    ...
                    if corrected and corrected != phrase and corrected not in self.seen_valid[12] and bip39_check(corrected):
        ...
        def on_found_electrum(self, phrase: str, path: str):
            ...
            try:
                ...
                pass
            try:
                with open(self.results_dir / f"electrum_valid_{n}.txt", "a", encoding="utf-8") as f:
                    f.write(f"{phrase} | {path}\n")
            except Exception:
                pass
            try:
    ...
    def main():
        if QT_IMPORT_ERROR is not None:
            _fatal_startup(f'Не удалось загрузить PySide6: {QT_IMPORT_ERROR}')
        else:
            _fatal_startup('PySide6 недоступен в этой среде исполнения.')
