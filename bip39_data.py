# Auto-generated minimal BIP-39 English wordlist and helpers.
# Source: https://github.com/bitcoin/bips/blob/master/bip-0039/english.txt
# The list is included verbatim to avoid external dependencies.
WORDLIST = [
    ...
    "near", "neck", "need", "negative", "neglect", "neither", "nephew", "nervous", "nest", "net",
    ...
    "vanish", "vapor", "various", "vast", "vector", "velvet", "vendor", "venture", "venue", "verb",
...
def check(phrase: str) -> bool:
    words = phrase.strip().split()
    length = len(words)
    if length not in ENTROPY_BITS:
        return False
    try:
        indexes = [INDEX_MAP[w] for w in words]
    except KeyError:
        return False
    bits = ''.join(f"{index:011b}" for index in indexes)
    ent_bits = ENTROPY_BITS[length]
    cs_bits = ent_bits // 32
    entropy_bits = bits[:ent_bits]
    checksum_bits = bits[ent_bits:ent_bits + cs_bits]
    entropy_bytes = int(entropy_bits, 2).to_bytes(ent_bits // 8, 'big')
    digest_bits = bin(int(sha256(entropy_bytes).hexdigest(), 16))[2:].zfill(256)
    expected = digest_bits[:cs_bits]
    return checksum_bits == expected


__all__ = ['WORDLIST', 'WORDSET', 'INDEX_MAP', 'check']
